...
import {
  View, Text, Modal, StyleSheet, Animated, Pressable, TouchableOpacity, Dimensions,
} from 'react-native'
...
export function NutritionSummarySheet({ visible, planName, macros, mealsCount, foodsCount, onClose }: Props) {
  ...
  return (
    <Modal visible={visible} transparent animationType="none" onRequestClose={onClose} statusBarTranslucent>
      <View style={s.overlay}>
        ...
        <Animated.View
          style={[s.sheet, { paddingBottom: Math.max(insets.bottom, 16) + 8, transform: [{ translateY }] }]}
        >
          <View style={s.handle} />

          <View style={s.header}>
            <View style={{ flex: 1, minWidth: 0 }}>
              <Text style={s.title}>Resumo nutricional</Text>
...
const s = StyleSheet.create({
  ...
  handle: { width: 40, height: 5, borderRadius: 3, backgroundColor: '#2A2A35', alignSelf: 'center', marginBottom: 16 },

  header: { flexDirection: 'row', alignItems: 'flex-start', gap: 12, marginBottom: 20 },
  title: { color: '#F0F0F5', fontSize: 18, fontWeight: '500' },
  subtitle: { color: '#8A8A9A', fontSize: 13, marginTop: 3 },

  calBlock: { flexDirection: 'row', alignItems: 'baseline', gap: 8 },
  calValue: { color: '#F0F0F5', fontSize: 30, fontWeight: '600', fontVariant: ['tabular-nums'] },
